...
public class CaesarCipher {
    ...
    public static char[] encrypt(final char[] line, final int shift) {
        ...
        for (int i = 0; i < result.length; i++) {
            int charShifted = result[i] + shift;
            if (charShifted > maxCharValue) {
                charShifted = minCharValue + shift % maxCharValue;
            }
                result[i] = (char) (result[i] + charShifted);
        }
    ...
    public static char[] decrypt(final char[] line, final int shift) {
        ...
        for (int i = 0; i < result.length; i++) {
            int charShifted = result[i] - shift;
            if (charShifted < minCharValue) {
                charShifted = maxCharValue - (shift % minCharValue);
            }
                result[i] = (char) (result[i] + charShifted);
        }
