...
public class Winner {
    ...
    public static String winner(final int firstScore, final int secondScore, final int thirdScore) {
        ...
        for (int runthroughs = 0; runthroughs < scores.length / 2 + 1; runthroughs++) {
            for (int i = 0; i < scores.length - 1; i++) {
                // swaps elements if the first one is lower than the next one
        ...
        for (int j = 0; j < scores.length; j++) { // compiles the string with commas
            result += Integer.toString(scores[j]);
