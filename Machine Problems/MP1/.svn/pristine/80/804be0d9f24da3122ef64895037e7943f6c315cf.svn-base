...
public final class QuizMaster {
    ...
    public static int computeScore(final int zipCode, final Boolean diversityAnswerCorrect,
        ...
        int score = 0;
        final int pointsGained = 10;
        final int extraCredit = 5;
        final int maxScore = 40;
        final int ilZipCode = 618;
        final int miZipCode = 481;
        final int zipStateCodeDivider = 100;

        final boolean[] answers = {diversityAnswerCorrect, illiacAnswerCorrect,
                mosaicAnswerCorrect, variableAnswerCorrect};

        for (int i = 0; i < answers.length; i++) { //gives points based on correct answers
            if (answers[i]) {
                score += pointsGained;
            }
        }
        // IL zip codes start with 618 and MI zip codes start with 481
        int newZipCode = zipCode / zipStateCodeDivider;
        if (score + extraCredit <= maxScore && newZipCode == ilZipCode) {
            score += extraCredit;
        }
        if (score < maxScore && newZipCode == miZipCode) {
            score = 0;
        } // if Michigan students get any wrong, set score to 0
        return score;
    }
