...
public class FindLine {
     ...
     */
    private static final String INTERACTIVE_TEXT_FILE = "Test.txt";
    ...
    public static void printMatchingLines(final String searchWord, final String corpus) {
        ...
        for (int i = 0; i < lines.length; i++) {
            String filteredSearchWord = searchWord.toLowerCase();
            String line = lines[i];
            String lowerCaseLine = line.toLowerCase();
            boolean validLine = false;

            char[] wordBoundary = {' ', '.', '!', '?', ',', ';', '(', ')', '-'};
            char[] wordBoundary2 = wordBoundary.clone();

            if (lowerCaseLine.length() < filteredSearchWord.length()) {
                lowerCaseLine = ""; // Filler Statement for check style appeasement
            } else if (lowerCaseLine.equals(filteredSearchWord)) {
                System.out.println(i + 1 + ": " + line);
            } else if (lowerCaseLine.contains(" " + filteredSearchWord + " ")) {
                System.out.println(i + 1 + ": " + line);
            } else if (lowerCaseLine.length() > filteredSearchWord.length()
                    && lowerCaseLine.substring(0, filteredSearchWord.length() + 1)
                            .equals(filteredSearchWord + " ")) {
                System.out.println(i + 1 + ": " + line);
            } else if (lowerCaseLine.length() > filteredSearchWord.length()
                    && lowerCaseLine
                            .substring(lowerCaseLine.length() - filteredSearchWord.length() - 1,
                                    lowerCaseLine.length())
                            .equals(" " + filteredSearchWord)) {
                System.out.println(i + 1 + ": " + line);
            } else if (lowerCaseLine.contains(filteredSearchWord)) { // Main Algorithm below

                for (int j = 0; j < wordBoundary.length; j++) {
                    char jChar = wordBoundary[j];
                    for (int k = 0; k < wordBoundary2.length; k++) {
                        char kChar = wordBoundary2[k];
                        if (lowerCaseLine.indexOf(filteredSearchWord) == 0) {
                            validLine = lowerCaseLine.charAt(filteredSearchWord.length()) == kChar;
                        } else {
                            validLine = lowerCaseLine.contains(jChar + filteredSearchWord + kChar);
                        }
