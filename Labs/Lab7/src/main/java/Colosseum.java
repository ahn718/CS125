...
public class Colosseum {
     ...
     */
    static final int MAX_NUM_ROUNDS = 50;
    ...
    public static Pokemon buildPokemon() {
        ...
        tempPokemon.name = scString.nextLine();

        System.out.println("How many hit points will it have? (1-50)");
        tempInt = scInt.nextInt();
        while (tempInt > MAX_HIT_POINTS) {
            System.out.println("Sorry. Hit points must be between 1 and 50");
            tempInt = scInt.nextInt();
        }
        tempPokemon.hitPoints = tempInt;

        System.out.println("Split fifty points between attack level and defense level");
        System.out.println("Enter your attack level (1-49):");
        tempInt = scInt.nextInt();
        while (tempInt > MAX_HIT_POINTS - 1) {
            System.out.println("Sorry. The attack level must be between 1 and 49");
            tempInt = scInt.nextInt();
        }
        tempPokemon.attackLevel = tempInt;

        int defenseLevel = MAX_HIT_POINTS - tempInt;

        System.out.println("Enter your defense level (1-" + defenseLevel + ")");
        tempInt = scInt.nextInt();
        while (tempInt > defenseLevel) {
            System.out.println("Sorry. The defense level must be between 1 and " + defenseLevel);
            tempInt = scInt.nextInt();
        }
        tempPokemon.defenseLevel = tempInt;
